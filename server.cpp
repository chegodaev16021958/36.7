...
void Server::incomingConnection(qintptr socketDescriptor)
{
    ...
    // соедин€ем сигнал о готовности данных от сокета со слотом slotReadyRead
    connect(socket, SIGNAL(readyRead()), this, SLOT(slotReadyRead()));
    // соедин€ем сигнал о разъединении со слотом deleteLater
    ...
    if (in.status() == QDataStream::Ok)
    {
        ...
        case 'M':
            ...
            *tempDateTime = QDateTime::currentDateTime();
            qDebug() << tempDateTime->toString("dd.Mћ hh:mm") << "new message from " << socketsMap[socket] << " to " << reciever << ": " << message;
        ...
        case 'N':
            ...
            if (modelUsers->rowCount())
            {
                // если такой логин зан€т, отвечаем 'E'
...
void Server::sendAllPreviosMessages()
{
    ...
    for (int i = 0; i < modelMessages->rowCount(); ++i)
    {
        ...
        *tempDateTime = tempData.toDateTime();
        qDebug() << senderName << reciever << message << tempDateTime->toString("dd.Mћ hh:mm");
